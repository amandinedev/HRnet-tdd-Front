...
const selectFromDropdown = async (label, optionText) => {
  ...
  await waitFor(() => {
    expect(
      screen.getByTestId(`input-select-options-${camelCaseLabel}`),
    ).toBeInTheDocument();
  });

  // Find and click option
  const optionsContainer = screen.getByTestId(
    `input-select-options-${camelCaseLabel}`,
  );
...
describe("EmployeeForm", () => {
  ...
  describe("Form Rendering", () => {
    it("renders all form fields with correct test IDs", () => {
      ...
      expect(
        screen.getByTestId("input-select-trigger-state"),
      ).toBeInTheDocument();
      expect(
        screen.getByTestId("input-select-trigger-department"),
      ).toBeInTheDocument();
  ...
  describe("Form Validation", () => {
    it("shows error messages for empty required fields", async () => {
      ...
      await waitFor(() => {
        ...
        const errors = screen.getAllByText(
          /Please fill in this field|Zip code is required|Please select a date|Please select an option/i,
        );
    ...
    it("shows error for invalid first name with numbers", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            /Only letters, hyphens, apostrophes, and spaces are allowed/i,
          ),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("Field Interactions", () => {
    ...
    it("allows selecting from dropdowns", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId("input-select-options-state"),
        ).toBeInTheDocument();
      });
      ...
      expect(
        options.querySelectorAll('button[role="option"]').length,
      ).toBeGreaterThan(1);
